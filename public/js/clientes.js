...
function renderizarClientes(lista) {
  ...
  tbody.innerHTML = lista.map(cliente => {
    ...
    const badgeClase = cliente.puntos >= 200 ? 'bg-success' : cliente.puntos >= 100 ? 'bg-danger' : cliente.puntos >= 50 ? 'bg-warning' : cliente.puntos >= 20 ? 'bg-info text-dark' : 'bg-secondary';
